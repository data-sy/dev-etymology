#!/usr/bin/env python3
"""
DevEtym 시스템 프롬프트 실측 프로브.

ClaudeAPIService.swift의 호출과 동일하게 (model, thinking, tools, tool_choice, system)
세팅하고 keyword 하나씩 호출하여 각 필드 길이와 본문을 출력한다.

사용법:
    export ANTHROPIC_API_KEY=sk-ant-...
    python Scripts/probe_prompt.py mutex idempotent bug cookie ping
"""
...
TOOLS: list[dict[str, Any]] = [
    {
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "summary": {"type": "string", "description": "20~30자 분량의 한 줄 요약"},
                "etymology": {
                    "type": "string",
...
def call(api_key: str, keyword: str) -> dict[str, Any]:
    body = json.dumps(
        {
            "model": CLAUDE_MODEL,
            "max_tokens": 4096,
...
def extract(payload: dict[str, Any]) -> tuple[str, dict[str, Any] | None]:
    """(tool_name, tool_input) 추출. tool_use 블록을 못 찾으면 ('unknown', None)."""
...
def report(keyword: str, payload: dict[str, Any]) -> None:
    name, inp = extract(payload)
    ...
    if name == "return_term_entry" and inp is not None:
        print(f"  tool: {name}")
        print(f"  category: {inp.get('category')}")
        print(f"  aliases: {inp.get('aliases')}")
        for f in ("summary", "etymology", "namingReason"):
            v = inp.get(f, "")
            print(f"  {f} ({len(v)}자): {v}")
    elif name == "return_possible_typo" and inp is not None:
        print(f"  tool: {name} → suggestion: {inp.get('suggestion')}")
    elif name == "return_not_dev_term":
        print(f"  tool: {name}")
    else:
        print(f"  tool: {name} — UNEXPECTED")
        print(json.dumps(payload, ensure_ascii=False, indent=2))


def main() -> int:
    if len(sys.argv) < 2:
        sys.exit("사용법: python Scripts/probe_prompt.py <keyword> [<keyword> ...]")
    api_key = os.environ.get("ANTHROPIC_API_KEY", "").strip()
    if not api_key:
        sys.exit("ANTHROPIC_API_KEY 환경변수가 비어있습니다")
    for kw in sys.argv[1:]:
        try:
            payload = call(api_key, kw)
        except urllib.error.HTTPError as e:
            print(f"\n=== {kw} === HTTPError {e.code}: {e.read().decode('utf-8', errors='replace')}", file=sys.stderr)
            continue
        except Exception as e:  # noqa: BLE001
            print(f"\n=== {kw} === ERROR: {e}", file=sys.stderr)
            continue
        report(kw, payload)
    return 0
