...
enum Theme {
    ...
    enum Typography {
        ...
        // MARK: 본문 (시스템 폰트 — Dynamic Type 추종)

        /// Settings 항목 라벨 (body 17pt)
        static let bodyLarge        = Font.system(.body)
        /// 일반 본문 — Detail namingReason, Onboarding 서브타이틀 (callout 16pt)
        static let body             = Font.system(.callout)
        /// Detail summary 전용 (callout 16pt)
        static let bodySub          = Font.system(.callout)
        /// 빈 상태·안내 타이틀 (headline 17pt semibold)
        static let bodyEmphasis     = Font.system(.headline)
        /// 강조 블록 내 본문 — Detail etymology(가장 길게 흐르는 본문) (callout 16pt)
        static let bodyBlock        = Font.system(.callout)
